...
require_once dirname(__DIR__, 2) . '/functions/custom_events.php';

// CSRF check
$csrf       = (string)($_SERVER['HTTP_X_CE_CSRF'] ?? '');
$csrfStored = (string)($_SESSION['bh_ce_csrf'] ?? '');
...
if ($action === 'toggle_enabled') {
    ...
    $result = bh_ce_toggle_event_enabled($userId, $eventId, $enabled);

    if ($result['ok'] && isset($result['bot_id']) && $result['bot_id'] > 0) {
        try {
...
if ($action === 'set_group') {
    ...
    $result = bh_ce_set_event_group($userId, $eventId, $groupName);
    echo json_encode($result);
    exit;
}
...
if ($action === 'delete_event') {
    ...
    if ($result['ok'] && $event && isset($event['bot_id'])) {
        try {
            if (function_exists('bh_notify_bot_reload')) {
                bh_notify_bot_reload((int)$event['bot_id']);
            }
        } catch (Throwable) {}
    }
