...
function bgvc_fail(int $code, string $msg): never
{
...
function bgvc_get_plain_token(PDO $pdo, int $botId, int $userId): string
{
    $stmt = $pdo->prepare(
...
function bgvc_curl_get(string $botToken, string $path): array
{
...
// Discord channel type 2 = GUILD_VOICE
const BGVC_CHANNEL_VOICE = 2;

try {
    $pdo      = bh_cc_get_pdo();
    $botToken = bgvc_get_plain_token($pdo, $botId, $userId);

    if ($botToken === '') {
        bgvc_fail(422, 'Bot-Token nicht verfügbar (verschlüsselt oder nicht gesetzt)');
    }

    // If no guild_id provided, fetch guild list first
    if ($guildId === '') {
        $res = bgvc_curl_get($botToken, '/users/@me/guilds');

        if ($res['http'] < 200 || $res['http'] >= 300 || !is_array($res['data'])) {
            bgvc_fail(502, 'Guilds konnten nicht geladen werden (HTTP ' . $res['http'] . ')');
        }
    ...
    $res = bgvc_curl_get($botToken, '/guilds/' . rawurlencode($guildId) . '/channels');

    if ($res['http'] < 200 || $res['http'] >= 300 || !is_array($res['data'])) {
        bgvc_fail(502, 'Channels konnten nicht geladen werden (HTTP ' . $res['http'] . ')');
    }
    ...
    usort($channels, static fn(array $a, array $b): int => strcasecmp($a['name'], $b['name']));
